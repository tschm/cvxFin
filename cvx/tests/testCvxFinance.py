...
def testLinearFalseBounds():
    assert_raises(AssertionError, Fin.solveLP, c=np.array([1.0]), A=[], bxl=np.array([2.0]), bxu=np.array([0.0]), bcl=[], bcu=[])


@with_setup(setup_func, teardown_func)
def testLinearBoundsTooShort():
    assert_raises(AssertionError, Fin.solveLP,
                  c=np.array([1.0, 2.0]),
                  A=[],
                  bxl=np.array([2.0]),
                  bxu=np.array([0.0]),
...
def testLinearNoConstraints():
    assert_raises(ArithmeticError, Fin.solveLP,
                  c=np.array([1.0]),
                  A=np.array([1.0]),
                  bxl=np.array([-np.inf]),
                  bxu=np.array([np.inf]),
...
def testQuadNegative():
    assert_raises(AssertionError, Fin.solveQPcon, c=__get_c(), A=__get_A(), Q=__get_Q(), qc=-17,
                  bxl=__get_bxl(), bxu=__get_bxu(), bcl=__get_bcl(), bcu=__get_bcu())


@with_setup(setup_func, teardown_func)
def testQuadInfeasible():
    assert_raises(ArithmeticError, Fin.solveQPcon, c=__get_c(), A=__get_A(), Q=__get_Q(), qc=1,
